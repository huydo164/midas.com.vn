...
class Product extends Model {
    ...
    public static function searchByCondition($dataSearch=array(), $limit=0, $offset=0, &$total){
        try{
            ...
            if (isset($dataSearch['product_min']) && $dataSearch['product_min'] != -1) {
                $query->where('product_price','>',  $dataSearch['product_min'] );
            }
            if (isset($dataSearch['product_max']) && $dataSearch['product_max'] != -1) {
                $query->where('product_price','<',  $dataSearch['product_max'] );
            }
            ...
            if (isset($dataSearch['product_size'])) {
                if(!empty($dataSearch['product_size']) && is_array($dataSearch['product_size'])){
                    $query->whereIn('product_size',  $dataSearch['product_size'] );
                }
            }

            if (isset($dataSearch['product_color'])) {
                if(!empty($dataSearch['product_color']) && is_array($dataSearch['product_color'])){
                    $query->whereIn('product_color',  $dataSearch['product_color'] );
                }
            }
