                ...
                <div class="row">
                    <div class="col-xs-12">
                            ...
                            <form id="frmSearch" method="GET" action="" class="frmSearch" name="frmSearch">
                                <div class="panel-body panel-search">
                                    <div class="form-group col-lg-2">
                                        ...
                                        <div>
                                            <input type="text" class="form-control input-sm" name="buy_name" @if(isset($search['buy_name']) && $search['buy_name'] !='')value="{{$search['buy_name']}}"@endif>
                                        </div>
                                    </div>
                                    <div class="form-group col-lg-2">
                                ...
                                <div class="panel-footer text-right">
                                    <a class="btn btn-default btn-sm" class="reset" href="{{route('admin.buy')}}"><i class="fa fa-recycle"></i>Bỏ lọc</a>
                                    <a class="btn btn-danger btn-sm" href="{{FuncLib::getBaseUrl()}}admin/buy/edit"><i class="ace-icon fa fa-plus-circle"></i>Thêm mới</a>
                        ...
                        @if(sizeof($data) > 0)
                            ...
                            @endif
                            <br>
                            <form id="formListItem" method="POST" action="{{FuncLib::getBaseUrl()}}admin/buy/delete" class="formListItem" name="txtForm">
                                <table class="table table-bordered table-hover">
                                    ...
                                    @foreach($data as $k=>$item)
                                        <tr>
                                            ...
                                            <td>
                                                <label class="pos-rel">
                                                    <input class="ace checkItem" name="checkItem[]" value="{{$item['buy_id']}}" type="checkbox">
                                                    <span class="lbl"></span>
                                                </label>
                                            </td>
                                            <td><a title="{{$item->buy_name}}" href="">{{$item['buy_name']}}</a></td>
                                            <td><a title="{{$item->buy_email}}" href="">{{$item['buy_email']}}</a></td>
                                            <td>{{ $item['buy_phone'] }}</td>
                                            <td>{{date('d/m/Y', $item['buy_created'])}}</td>
                                            <td>
                                            ...
                                            <td>
                                                <a href="{{FuncLib::getBaseUrl()}}admin/buy/edit/{{$item['buy_id']}}" title="Cập nhật">
